	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			if is_mouse_over_active_cell(mouse_pos):
				...
				hide_active_icon_for_drag()
				
				# Настраиваем размеры слоев превью
			...
			if is_dragging_internal:
				...
				internal_drag_preview.hide()
				var moved = try_move_active_cell(mouse_pos)
				
				if not moved:
					show_active_icon_after_drag()
...
func _place_item_smartly(item_id: int, mouse_pos: Vector2) -> Control:
	...
	# ИСПРАВЛЕНИЕ: Точный центр вычисляется от самой иконки
	target_icon_node.pivot_offset = target_icon_node.size / 2.0
...
func try_move_active_cell(mouse_pos: Vector2) -> bool:
	...
	if target_cell and target_cell != active_cell:
		...
		# Обязательно вычисляем центр для новой ячейки
		icon_target.pivot_offset = icon_target.size / 2.0
		icon_target.show()

		icon_active.texture = null
		icon_active.hide()
		icon_active.rotation_degrees = 0
		icon_active.pivot_offset = icon_active.size / 2.0
...
func _on_btn_rotate():
	if active_cell:
		...
		var tw = create_tween()
		tw.tween_property(icon, "rotation_degrees", icon.rotation_degrees + 90.0, 0.15).set_trans(Tween.TRANS_SINE)
